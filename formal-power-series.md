...
A: 普通の積の和典型とは違って、追加したボールついて「どの箱に追加したか」という情報(ラベル)が必要だから。

たとえば、箱$1$に$3$個、箱$2$に$1$個、箱$3$に$4$個追加する場合の数は(順番の違いがあるので)1通りではない。
1と書かれたボールを3個、2と書かれたボールを1個、3と書かれたボールを4個並び替える方法の数に対応している。 これは多項係数で表せて、
