...
- $A_1(A_2+2)$の項は、箱$1$に$0$個、箱$2$に$2$個追加する場合に対応している。
  - あってそう
- $(A_1 + 1)(A_2 + 1)$は箱$1$に$1$個、箱$2$に$1$個追加する場合に対応しているしている
...
A: 箱$1$に$3$個、箱$2$に$1$個、箱$3$にに$4$個追加する場合の数は

$\frac{(3+1+4)!}{3!1!4!}$になる。このように多項係数の重みをつけなければいけなかったから。

そう考えると、$f_i = A_i + \frac{(A_1 + 1)}{1!} x + \frac{(A_i + 2)}{2!} x^2 + \ldots$ として、 $\prod_{i=1}^{N} f_i$ の $x^K$ の係数を求めればよさそう。 (先に学んだとおり、EGFの積には係数の重みに多項係数がつくのでので)
