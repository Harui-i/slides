...
(結論を先に書くと、$\varphi_A(x)=x^K - c_1 x^{K-1} - \dots - c_{K-1} x^1 - c_{K}$ )
()
...
mod $\varphi_A(x)$ 上で$x^N$を計算すればよい。FFTを活用して頑張ると割り算に$O(K\log K)$かかり、繰り返し二乗法で掛け算を$O(\log N)$回やるので $O(K \log K \log N)$ で求められる。
...
- https://judge.yosupo.jp/problem/kth_term_of_linearly_recurrent_sequence (Library Checker, まさにこのスライドで扱ったような問題。 mod $998244353$で求める問題で、 $O(K \log K \log N)$ が想定で、$O(K^2 \log N)$ は通らない。)
